...
const SelectProfileType = ({onClose}) => {
  ...
  const onSave = () => {
    setPreferenceDispatch({type: SET_PREFERENCE_PROFILE, payload: Number(preference)})
    onClose()
  }
 return (
  <div className="cardSelect">
   <div className="cardSelect__form--select">
    ...
    <select onChange={setPreference}>
     <option value="1" selected={employeer ? true : false} >Worker</option>
     <option value="0" selected={employeer ? false : true}>Employeer</option>
    </select>
   </div>
   <div className="cardSelect__form--buttons">
    <Button active onClick={onSave}> Change</Button>
